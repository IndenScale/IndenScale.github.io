...
- **输入**：上一轮模型学习阶段收集的失败日志。
- **过程**：在启动阶段，由领域专家与软件工程师手工设计初始 F 和 S。系统稳定后，失败日志聚类器识别常见错误模式，通过模板或 LLM 辅助生成新的验证器或微调框架。由人类审核后提交更新。
- **输出**：更新后的 Jargon 配置 (F′, S′)，覆盖范围扩大。
...
Jargon 的定义本身是一项软件工程任务。早期需兼顾领域与工程的专家介入；框架成熟后可通过下述机制半自动化：
...
该混合回路模仿人类行话的演化：由创始人积极协商，随后被编纂与传授，偶因异常积累而被改革。
...
2. **跨领域推广**：将 Jargon RL 推广到芯片布局验证、合规审查、物流调度等其他具有丰富结构化知识的领域。
3. **审查项硬化**：探索将残留的软性审查项逐步硬化的一般方法，最终实现全自动、零主观的领域对齐闭环。

我们相信，使验证器与模型共同生长，将是通向可靠、可扩展对齐的重要一步。
